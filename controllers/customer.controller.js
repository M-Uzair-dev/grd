...
exports.getAllCustomers = async (req, res) => {
  try {
    const customers = await Customer.find().populate("partnerId", "name email");
  ...
  try {
    ...
    if (req.role === "partner") {
      ...
      if (customer.partnerId._id.toString() !== req.user._id.toString()) {
        return res
          .status(403)
          .json({ message: "Not authorized to access this customer" });
      }
    ...
    if (partnerId && partnerId !== customer.partnerId._id.toString()) {
      ...
      if (newPartner.adminId.toString() !== req.user._id.toString()) {
        return res.status(403).json({
          message: "Not authorized to assign customer to this partner",
        });
      }
...
exports.deleteCustomer = async (req, res) => {
  try {
    ...
    if (!customer.partnerId || !customer.partnerId.adminId) {
      return res
        .status(400)
        .json({ message: "Invalid partner data for this customer" });
    }
    ...
    try {
      ...
      await Report.deleteMany({
        $or: [
          ...
          {
            unitId: {
              $in: await Unit.find({ customerId: customer._id }).distinct(
                "_id"
              ),
            },
          },
      ...
      return res
        .status(500)
        .json({ message: "Failed to delete customer data. Please try again." });
    }
  } catch (error) {
